...
// 平台请求头（与官网前端一致的客户端标识，可帮助平台正确识别请求来源）
...
// 平台侧接口：users/current 可读取账户余额预警设置（balance_alert[币种].alert_bound）
...
// 白饭档位判定（Issue #34）：balance >= 2*底线 → 满碗；>= 底线 → 半碗；否则空碗。
// 底线：手动开关「使用平台预警阈值」开启且拉到有效平台值时用平台值，否则用菜单手动值（¥，0=恒满碗）
function computeRiceLevel() {
  ...
  var b = state.balance
  if (b === null || b === undefined || !isFinite(Number(b))) return riceLevel || 'full'
  var num = Number(b)
...
function apply(ctx) {
    ...
    // 白饭图标按档位读取（full/half/empty），带内存缓存
    const riceBytesCache = {}
    function loadRice(level) {
    ...
    // 读取 DeepSeek 平台 users/current 的 balance_alert[币种]（enabled 开关 + alert_bound 阈值），
    // 作为白饭图标「余额底线」的推荐值；无令牌/接口失败时前端回退菜单手动阈值。
    let alertCache = null
    function isAlertAuthFail(msg) {
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, threshold, usePlatformAlert) {
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.threshold, parsed.usePlatformAlert)
