...
- **白饭余额图标**（`assets/rice/{full,half,empty}.png`，Issue #34）：鲸鱼左侧底部常驻，三态显示**当前展示账户**充裕度（跟随套餐切换）。有货币余额时：余额 ≥ 2×底线 → 满碗；底线 ≤ 余额 < 2×底线 → 半碗；余额 < 底线 → 空碗。订阅窗口（OpenCode Go / Z.AI / Grok）按主窗口剩余配额：剩余 >50% 满碗、>25% 半碗、否则空碗（用量 51% → 半碗）。DeepSeek 底线来源两种，菜单自由切换：
...
- 点击鲸鱼 → 气泡弹出显示正常内容（**跟随模型开启时 = 当前会话所用模型的账户额度**，关闭跟随后按「点击切换账户」开关轮换），总时长 **5 秒**自动收起。
